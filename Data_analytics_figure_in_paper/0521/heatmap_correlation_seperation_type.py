...
df[to_num] = df[to_num].apply(pd.to_numeric, errors='coerce').round(4)


# 4) Group by the `well-separated` column instead of numeric bins
group_labels = df["well-separated"].unique().tolist()   # distinct well-separated values

# 5) Compute mean Kendall’s τ per method per `well-separated` group
...
group_corr = {
    grp: compute_kendall_for_subset(df[df["well-separated"] == grp])
...
heat.set_ylabel("Well-Separated Value", fontsize=14)
heat.set_title("Impact of ‘well-separated’ on Rank Correlation",
               fontsize=16, pad=12)
