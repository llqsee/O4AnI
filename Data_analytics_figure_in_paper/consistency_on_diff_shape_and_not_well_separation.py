...
def classify_shape(shape_list):
    if isinstance(shape_list, list):
        if all(s == 'cluster' or s == 'lip_cluster' for s in shape_list):
            return 'only_cluster'
        else:
...
bars1 = ax.barh(y - height/2, cluster_scores, height, label='Cluster shapes & not_well-separated', color='cornflowerblue')
bars2 = ax.barh(y + height/2, other_scores, height, label='Mixed shapes & not_well-separated', color='darkorange')
